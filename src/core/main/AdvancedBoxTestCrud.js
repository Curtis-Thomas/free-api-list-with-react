...
import { Box, Typography, Button, Input, Select, MenuItem } from "@mui/material";
...
const AdvancedBoxTestCrud = ({ url }) => {
    ...
    const fetchAPI = async () => {
        try {
            ...
            if (crudOption) {
                ...
                if (urlValue) { options["url"] = urlValue + endpointValue } // URL to send the request

                if (headers) { options["headers"] = JSON.parse(headers) } // Parsed headers in JSON format

                if (body) { options["data"] = body } // Request body (if applicable)
    ...
    return (
        <Box
            sx={{ border: "solid 1px #0077ba", padding: 2, margin: 2, minHeight: "20vh" }}
        >
